...
export default function ProtocolsPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setGenerationResult({
        success: false,
        message: error.message || '❌ Failed to generate protocols. Check API key in Vercel settings.'
      })
    } finally {
      setGenerating(false)
    }
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 max-w-md w-full mx-4">
            ...
            <p className="text-gray-600 mb-4">
              AI will generate communication protocols with step-by-step instructions, examples, and explanations.
            </p>

            <div className="mb-6">
              <label className="block text-sm font-medium text-gray-700 mb-2">
                How many protocols to generate?
              </label>
              <select
                value={generateCount}
              ...
              >
                <option value={1}>1 protocol</option>
                <option value={2}>2 protocols</option>
                <option value={3}>3 protocols</option>
                <option value={5}>5 protocols</option>
                <option value={10}>10 protocols</option>
                <option value={25}>25 protocols</option>
                <option value={50}>50 protocols</option>
              </select>
            </div>

            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-6">
              <p className="text-sm text-blue-800 mb-2">
                <strong>Note:</strong> Generated protocols will be saved as drafts. Review and publish when ready.
              </p>
              {generateCount >= 10 && (
